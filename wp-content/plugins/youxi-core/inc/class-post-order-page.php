...
if( ! class_exists( 'Youxi_Post_Order_Page' ) ) :
	...
	class Youxi_Post_Order_Page extends Youxi_Post_Page {
		...
		public function __construct( $page_title, $menu_title, $menu_slug, $capability = 'edit_others_posts', $callback = null ) {
		...
		public function admin_enqueue_scripts( $hook ) {
			...
			if( parent::admin_enqueue_scripts( $hook ) ) {
				...
				wp_enqueue_style( 'youxi-post-order', YOUXI_CORE_URL . 'admin/assets/css/youxi-post-order.css', array(), YOUXI_CORE_VERSION );
				wp_enqueue_script( 'youxi-post-order', YOUXI_CORE_URL . 'admin/assets/js/youxi.post-order.js', array( 'jquery', 'jquery-ui-sortable', 'wp-util' ), YOUXI_CORE_VERSION, true );

				wp_localize_script( 'youxi-post-order', '_youxiPostOrder', array(
		...
		public function save_order() {
			...
			check_ajax_referer( 'youxi-post-order-nonce', 'nonce' );

			if( ! current_user_can( 'edit_posts' ) ) {
				wp_die( esc_html__( 'You\'re not allowed to do this action.', 'youxi' ) );
			}

			if( ! empty( $_POST['menu_order'] ) ) {

				foreach( $_POST['menu_order'] as $menu_order => $ID ) {
