		...
		<div class="content-wrap">
			...
			?><div <?php echo $gridlist_attributes ?>>
				...
				<?php if( $grid_settings['show_filter'] ):
					...
					$terms = get_terms( youxi_portfolio_tax_name(), array(
						'include' => $grid_settings['include'], 
						'orderby' => $grid_settings['filter_orderby'], 
						'order'   => $grid_settings['filter_order']
					) );
					...
					?><article <?php post_class( array_merge( array( 'grid' ), $post_class ) ); ?> itemscope itemtype="http://schema.org/Article">
						...
						<div class="grid-inner">
							...
							<div class="portfolio-info">
								...
								<a class="portfolio-info-link" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>" itemprop="url"><?php

									the_title( '<h3 class="entry-title portfolio-title" itemprop="name">', '</h3>' );

									if( 'taxonomy' == $grid_settings['meta_text'] ) : 

										if( $post_terms ) : 
											echo '<p class="portfolio-meta">' . join( ', ', $post_terms ) . '</p>';
										endif;

									elseif( 'excerpt' == $grid_settings['meta_text'] ) : 

										if( $post->post_excerpt ) : 
											echo '<p class="portfolio-meta">' . esc_html( $post->post_excerpt ) . '</p>';
										endif;
