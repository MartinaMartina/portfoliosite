 ...
 */
if( ! class_exists( 'Youxi_Shortcode_Animation' ) ) :
		...
		public function shortcode_fieldsets( $fieldsets ) {
			...
			if( ! isset( $fieldsets[ $animation_fieldset ] ) ) {
				$fieldsets[ $animation_fieldset ] = apply_filters( 'youxi_shortcode_animation_fieldset', array(
					'id' => 'animation', 
					'title' => esc_html__( 'Animation', 'youxi' )
				));
			}
		...
		public function shortcode_atts( $atts, $tag ) {
			...
			if( isset( $this->shortcodes[ $tag ] ) && is_array( $this->shortcodes[ $tag ] ) ) {
				...
				foreach( $this->shortcodes[ $tag ] as $key => $title ) {
					...
					$fieldsets[ $key ] = array(
						...
						'fields' => array(
							'name' => array(
								'type' => 'select', 
								'label' => esc_html__( 'Animation', 'youxi' ), 
								'description' => esc_html__( 'Choose here the animation.', 'youxi' ), 
								'choices' => array_combine( $this->animations, $this->animations ), 
								'std' => 'flipInX'
							), 
							'duration' => array(
								'type' => 'number', 
								'label' => esc_html__( 'Duration', 'youxi' ), 
								'description' => esc_html__( 'Specify the duration of the animation (-1 means unspecified).', 'youxi' ), 
								'min' => -1, 
								'step' => 10, 
								'std' => -1
							), 
							'delay' => array(
								'type' => 'number', 
								'label' => esc_html__( 'Delay', 'youxi' ), 
								'description' => esc_html__( 'Specify the delay before the animation begins (-1 means unspecified).', 'youxi' ), 
								'min' => -1, 
								'step' => 10, 
								'std' => -1
							)
				...
				$_atts[ $atts_name ] = array(
					...
					'description' => esc_html__( 'Configure here the animation properties.', 'youxi' ), 
