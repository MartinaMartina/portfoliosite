...
;(function( $, window, document, undefined ) {
	...
	$.widget( 'youxi.mediauploader', {
		...
		_handleSelection: function() {
			...
			selection.map( function( attachment ) {
				var data = {
					id: attachment.id, 
					url: this._getAttachhmentImageUrl( attachment, 'thumbnail' ), 
					fieldName: this.options.fieldName, 
					fieldNamePostfix: this.options.multiple ? '[]' : ''
				};

				if( 'url' == this.options.returnType ) {
					data.id = this._getAttachhmentImageUrl( attachment, this.options.returnUrlSize );
				}
		...
		_getAttachhmentImageUrl: function( attachment, attachmentSize ) {
			var sizes = attachment.attributes.sizes || {};

			if( _.has( sizes, attachmentSize ) ) {
				return sizes[ attachmentSize ].url;
			}

			return attachment.attributes && attachment.attributes.url || '';
		}, 
	...
	$.widget( 'youxi.fileuploader', {
		...
		_handleSelection: function() {
			var selection = this._mediaFrame.state().get( 'selection' );
			var returnType = this.options.returnType;

			returnType = ( 'id' == returnType || 'url' == returnType ) ? returnType : 'id';
			...
			selection.map( function( attachment ) {
				if( attachment && attachment.attributes ) {
					this._feedback( attachment.attributes.url, attachment.attributes[ returnType ] );
				}
			}, this );
