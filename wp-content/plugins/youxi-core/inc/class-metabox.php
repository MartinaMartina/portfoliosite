	...
	final class Youxi_Metabox {
		...
		public function admin_enqueue_scripts( $hook ) {
			...
			if( 'post.php' == $hook || 'post-new.php' == $hook ) {
				...
				if( 'page' == $post_type && ! empty( $this->page_template ) ) {
					wp_enqueue_script( 'youxi-page-template', YOUXI_CORE_URL . 'admin/assets/js/youxi.page-template.js', array( 'jquery' ), YOUXI_CORE_VERSION, true );
				}
			}
		}
		...
		public function save_fields( $post_id, $data ) {
			...
			if( check_admin_referer( "youxi_mb_{$post_type}_{$this->id}", "{$post_type}_{$this->id}_nonce" ) ) {

				$proceed_with_save = ! empty( $data[ $this->id ] );

				/* Don't save inactive page template metaboxes */
				if( 'page' == $post_type && ! empty( $data[ $this->id ]['page_template'] ) ) {
					$page_template = $data[ $this->id ]['page_template'];
					if( "$page_template.php" != get_post_meta( $post_id, '_wp_page_template', true ) ) {
				...
				if( $proceed_with_save ) {
					...
					$meta_data = array_intersect_key( $data[ $this->id ], $this->fields );

					// Sanitize the data
					$meta_data = $this->get_form( $post_type )->sanitize( $meta_data );
					...
					if( $this->as_array ) {
						...
						foreach( $meta_data as $meta_key => $meta_value ) {
							...
							if( $this->fields[ $meta_key ]['scalar'] ) {
						...
						if( ! empty( $meta_data ) ) {
							update_post_meta( $post_id, $this->id, $meta_data );
						}
		...
		public function get_form( $post_type ) {
			...
			if( ! empty( $this->form_cache[ $metabox_id ] ) ) {
			...
			/* Add a hidden field for page template metaboxes */
			if( 'page' == $post_type && ! empty( $this->page_template ) ) {
				$before_fields .= '<input type="hidden" name="' . esc_attr( "{$post_type}[{$this->id}][page_template]" ) . '" value="' . esc_attr( $this->page_template ) . '">';
			}
				...
				'field_attr' => array(
					'class' => array(
						'youxi-form-large' => array(
							'type' => array( 'text', 'textarea', 'code', 'url', 'number', 'select', 'iconchooser' )
						)
					)
				), 
