...
;(function( $, window, document ) {
	...
	$.extend( Helium, {
		...
		teardown: function( context ) {
			...
			if( $.fn.isotope ) {
				$( '.gallery', context ).isotope( 'destroy' );
			}
			...
			if( $.magnificPopup ) {
				...
				/* Remove removalDelay to make sure popup is closed */
				if( $.magnificPopup.instance.st ) {
					$.magnificPopup.instance.st.removalDelay = null;
				}
			...
			if( $.fn.ajaxFormUnbind ) {
				...
				$( 'div.wpcf7 > form', context ).ajaxFormUnbind().unbind( '.form-plugin' );
			}
			...
			if( $.Youxi && $.Youxi.Shortcode ) {
				$.Youxi.Shortcode.teardown( context );
			}
		}, 
				...
				Helium.AjaxNavigator.init({
					...
					stateChange: function( url, callback ) {
						...
						var menu, 
							...
							responseWpAdminBar = responseObj.find( '#wpadminbar' );

						// Replace WordPress admin bar
						var currentAdminBar = $( '#wpadminbar' );
						if( currentAdminBar.length && responseWpAdminBar.length ) {
							currentAdminBar.replaceWith( responseWpAdminBar );
						}
