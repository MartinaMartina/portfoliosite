 ...
 * Youxi JS Class
 *
 * This class provides a set of useful JS manipulation methods
 *
 * @package   Youxi Core
...
if( ! class_exists( 'Youxi_JS' ) ) :
	...
	final class Youxi_JS {
		...
		public static function minify( $js_code, $args = array() ) {
			...
			if( isset( $js_cache[ $js_hash ] ) && is_string( $js_cache[ $js_hash ] ) ) {
			...
			$closure_compiler_url = apply_filters( 'youxi_js_closure_compiler_url', 'http://closure-compiler.appspot.com/compile' );
			...
			if( ! is_wp_error( $response ) && 200 == wp_remote_retrieve_response_code( $response ) ) {
				...
				// Return the original javascript code in case of errors
				if( is_null( $response_obj ) || ! isset( $response_obj['compiledCode'] ) || isset( $response_obj['errors'], $response_obj['serverErrors'] ) ) {
					return $js_code;
				}
