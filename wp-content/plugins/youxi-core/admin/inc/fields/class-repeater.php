	...
	class Youxi_Repeater_Form_Field extends Youxi_Form_Field {
		...
		public function __construct( $scope, $options, $allowed_hooks = array() ) {
			...
			$this->default_options = array_merge( $this->default_options, array(
				...
				'fields'           => array(), 
				'depth'            => 0, 
				'add_text'         => esc_html__( 'Add', 'youxi' ), 
				'edit_text'        => esc_html__( 'Edit', 'youxi' ), 
				'delete_text'      => esc_html__( 'Delete', 'youxi' ), 
				'preview_template' => false, 
				'confirm_delete'   => esc_html__( 'Are you sure you want to remove this item?', 'youxi' )
			));
		...
		public function sanitize( $data ) {
			...
			return array_values( array_map( array( $this, 'sanitize_helper' ), $data ) );
		}
		...
		public function get_the_field( $value, $attributes = array() ) {
			...
			$o = '<div id="' . $this->get_the_ID() . '"' . Youxi_Form::render_attr( $attributes ) . '>';
				...
				if( ! $this->get_option( 'depth' ) ):
					$o .= $this->get_templates();
				endif;

				$o .= '<fieldset>';

					$o .= '<div class="youxi-repeater-fields">';

						for( $this->index = 0, $count = count( $value ); $this->index < $count; $this->index++ ):
							$o .= $this->get_template( $value[ $this->index ], false );
						endfor; 

						for( ; $this->index < $minimum_rows; $this->index++ ):
							$o .= $this->get_template( null, false );
						endfor;

					$o .= '</div>';

					$o .= '<div class="youxi-repeater-controls">';

						$o .= '<button type="button" class="button button-large button-primary" data-action="add">';

							$o .= esc_html( '+ ' . $this->get_option( 'add_text' ) );
