	...
	class Youxi_Code_Form_Field extends Youxi_Form_Field {
		...
		public function enqueue( $hook ) {
			...
			if( parent::enqueue( $hook ) ) {
				...
				wp_register_script( 'codemirror', self::field_assets_url( "plugins/codemirror/codemirror{$suffix}.js" ), array(), '5.14.2', true );

				if( 'javascript' == $this->get_option( 'mode' ) ) {
					wp_enqueue_script( 'codemirror-javascript', self::field_assets_url( "plugins/codemirror/mode/javascript{$suffix}.js" ), array( 'codemirror' ), '5.14.2', true );
				} elseif( 'css' == $this->get_option( 'mode' ) ) {
					wp_enqueue_script( 'codemirror-css', self::field_assets_url( "plugins/codemirror/mode/css{$suffix}.js" ), array( 'codemirror' ), '5.14.2', true );
				}

				wp_enqueue_style( 'codemirror', self::field_assets_url( 'plugins/codemirror/codemirror.css' ), array(), '5.14.2' );
				wp_enqueue_script( 'youxi-code-editor', self::field_assets_url( "js/youxi.form.code{$suffix}.js" ), array( 'youxi-form-manager' ), YOUXI_CORE_VERSION, true );
			}
		}
