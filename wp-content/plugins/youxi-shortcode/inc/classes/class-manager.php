...
if( ! class_exists( 'Youxi_Shortcode_Manager' ) ) :
	...
	final class Youxi_Shortcode_Manager {
		...
		public function get_shortcode_editor() {
			...
			if( isset( $_POST['shortcode'] ) ) {
				...
				if( ! current_user_can( 'edit_posts' ) ) {
					wp_die( esc_html__( 'You\'re not allowed to make this request.', 'youxi' ) );
				}
				...
				$shortcode_object = $this->get_shortcode( $shortcode );

				if( is_a( $shortcode_object, 'Youxi_Shortcode' ) ) {

					if( ! $shortcode_object->internal ) {
						...
						$editor = new Youxi_Shortcode_Editor( $shortcode_object, $data );

						$output['title'] = sprintf( '%s %s', $data ? esc_html__( 'Update', 'youxi' ) : esc_html__( 'Insert', 'youxi' ), $shortcode_object->label );
						...
						wp_send_json_error( new WP_Error( 'internal_shortcode', esc_html__( 'You can\'t edit an internal shortcode.', 'youxi' ) ) );
					}

				} else {

					wp_send_json_error( new WP_Error( 'invalid_shortcode', sprintf( esc_html__( 'The shortcode \'%s\' does not exist.', 'youxi' ), $shortcode ) ) );
				}
			}

			wp_send_json_error( new WP_Error( 'invalid_request', esc_html__( 'Invalid request.', 'youxi' ) ) );
		}
		...
		public function admin_enqueue_scripts( $hook ) {
			...
			wp_register_script( 'serialize-json', YOUXI_SHORTCODE_URL . "assets/admin/plugins/serializejson/jquery.serializejson{$suffix}.js", array( 'jquery' ), '1.2.3', true );

			wp_register_script( 'youxi-shortcode-editor', YOUXI_SHORTCODE_URL . "assets/admin/js/youxi.shortcode.editor{$suffix}.js", array( 'media-views', 'serialize-json' ), YOUXI_SHORTCODE_VERSION, true );

			/* Register shortcode admin helper */
			wp_register_script( 'youxi-shortcode', YOUXI_SHORTCODE_URL . "assets/admin/js/youxi.shortcode{$suffix}.js", array( 'youxi-shortcode-editor', 'youxi-form-manager', 'underscore' ), YOUXI_SHORTCODE_VERSION, true );

			/* Register shortcode admin styles */
			wp_register_style( 'youxi-shortcode', YOUXI_SHORTCODE_URL . "assets/admin/css/youxi.shortcode{$suffix}.css", array( 'youxi-form', 'media-views' ), YOUXI_SHORTCODE_VERSION, 'screen' );

			/* Attach setting variables to the script */
			wp_localize_script( 'youxi-shortcode', 'youxiShortcodeSettings', apply_filters( 'youxi_shortcode_js_vars', array(
	...
	/* ]]> */
		...
		public function enqueue() {
			...
			/* Whether to enqueue the frontend assets */
			if( ! is_admin() && apply_filters( 'youxi_shortcode_enqueue_frontend_assets', true ) ) {

				/* Enqueue frontend script */
				wp_enqueue_script( 'youxi-shortcode-frontend', YOUXI_SHORTCODE_URL . "assets/frontend/js/youxi.shortcode-frontend{$suffix}.js", array( 'jquery' ), YOUXI_SHORTCODE_VERSION, true );
			}

			/* Whether to enqueue the default assets */
			if( apply_filters( 'youxi_shortcode_enqueue_bootstrap', true ) ) {

				/* Tether for Bootstrap Tooltip plugin */
				wp_enqueue_script( 'tether', YOUXI_SHORTCODE_URL . "assets/frontend/plugins/tether/tether{$suffix}.js", array(), '1.3.2', true );

				/* Enqueue Bootstrap script */
				wp_enqueue_script( 'youxi-shortcode-bootstrap', YOUXI_SHORTCODE_URL . "assets/frontend/bootstrap/js/bootstrap{$suffix}.js", array( 'jquery' ), '4.0.0-alpha6', true );

				/* Enqueue Bootstrap style */
				wp_enqueue_style( 'youxi-shortcode-bootstrap', YOUXI_SHORTCODE_URL . "assets/frontend/bootstrap/css/bootstrap{$suffix}.css", array(), '4.0.0-alpha6', 'screen' );
			}

			/* If we're on any single post page */
			if( is_single() || is_page() ) {
				...
				if( isset( $matches[1] ) ) {
					...
					$this->enqueue_shortcodes( $matches );
				}
			}
		}
		 ...
		 * Enqueue shortcodes
		 */
		public function enqueue_shortcodes( $enqueue = array() ) {

			foreach( $this->shortcodes as $tag => $shortcode ) {
				if( empty( $enqueue ) || in_array( $tag, $enqueue ) ) {
					$shortcode->enqueue();
				}
			}
		}
