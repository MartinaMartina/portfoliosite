	...
	abstract class Youxi_Form_Field {
		...
		public function get_the_group_open( $tag = 'div', $attributes = array() ) {
			...
			if( $this->get_option( 'criteria' ) ) {
				...
				$attributes['data-criteria'] = $criteria['condition'];
		...
		public function get_the_control_open( $tag = 'div', $attributes = array() ) {
			return '<' . ( $this->control_tag = $tag ) . Youxi_Form::render_attr( $attributes ) . '>';
		}
		...
		public function enqueue( $hook ) {
			...
			/* Restrict form fields only to post.php or post-new.php */
			if( 'post.php' == $hook || 'post-new.php' == $hook ) {
			
				$suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';

				wp_enqueue_script( 'youxi-form-manager', self::field_assets_url( "js/youxi.form.manager{$suffix}.js" ), array( 'jquery', 'underscore' ), YOUXI_CORE_VERSION, true );

				wp_enqueue_style( 'youxi-form', self::field_assets_url( "css/youxi.form{$suffix}.css" ), array(), YOUXI_CORE_VERSION, 'screen' );

				if( ! wp_style_is( 'font-awesome' ) ) {
					wp_enqueue_style( 'font-awesome', '//maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css', array(), '4.7.0', 'screen' );
				}
				...
				switch( $field_type ) {
					...
					case 'number':
						if( ! class_exists( 'Youxi_Number_Form_Field' ) ) {
							require( plugin_dir_path( __FILE__ ) . 'fields/class-number.php' );
						}
						return new Youxi_Number_Form_Field( $scope, $options );
					case 'postselect':
						if( ! class_exists( 'Youxi_Postselect_Form_Field' ) ) {
							require( plugin_dir_path( __FILE__ ) . 'fields/class-postselect.php' );
						}
						return new Youxi_Postselect_Form_Field( $scope, $options );
					case 'radio':
		...
		public static function generate_ID( $scope, $name ) {
			return implode( '_', array_filter( preg_split( '/(\[|\])/', self::generate_name( $scope, $name ) ) ) );
		}
