	...
	*/
	...
	builder.view.Column = builder.view.Panel.extend({
		...
		initialize: function() {
			...
			_.defaults( this.options, { 'size': parent.controller.get( 'slots' ) } );
		...
		refresh: function() {
			...
			var cssObj  = { 'width': '', 'marginLeft': '' }, 
				rowSize = builder.settings.rowSize, 
				size    = this.controller.get( 'size' ), 
				offset  = this.controller.get( 'offset' );

			if( $.isNumeric( size ) ) {
				this.title.set( builder.controller.Column.getTitle( size ) );
				cssObj[ 'width' ] = ( 100 * parseInt( size ) / rowSize ) + '%';
			}

			if( $.isNumeric( offset ) ) {
				cssObj[ 'marginLeft' ] = ( 100 * parseInt( offset ) / rowSize ) + '%';
			}
