...
Changelog:
2.5 - 09/05/2016
- Improvement: Removed the simplified columns mode for Youxi Shortcode v3.x
- Improvement: Minor code rewrites to use the new column features on Youxi Shortcode v4.0
- Improvement: Column with offsets is now visualized
- Requirement: Plugin requires at least Youxi Shortcode v4.0
...
- Improvement: Remove hard coded builder icon definitions
...
function youxi_builder_plugins_loaded() {
	...
	if( ! defined( 'YOUXI_SHORTCODE_VERSION' ) ) {
		...
		if( version_compare( YOUXI_SHORTCODE_VERSION, '4.0', '<' ) ) {
			if( ! class_exists( 'Youxi_Admin_Notice' ) ) {
				require( plugin_dir_path( __FILE__ ) . 'class-admin-notice.php' );
			}
			Youxi_Admin_Notice::instance()->add_error( __FILE__, __( 'The current version of this plugin requires at least Youxi Shortcode 4.0 to work.', 'youxi' ) );
			return;
		}
	}
	...
	define( 'YOUXI_BUILDER_VERSION', '2.5' );
