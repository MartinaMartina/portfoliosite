	...
	final class Youxi_Form {
		...
		public function process_fields( $fields ) {
			...
			foreach( $fieldsets as $key => $fieldset ) {
				...
				foreach( $fieldset as $id => $setting ) {

					if( ! in_array( $id, array( 'id', 'title', 'fields', 'active' ) ) ) {
		...
		public function parse_args( $args ) {
			...
			$keys = array_keys( get_class_vars( get_class() ) );
		...
		public function has_fieldsets() {
			return isset( $this->_form_objects[0] ) && $this->is_fieldset( $this->_form_objects[0] );
		}
		...
		public function compile( $values = array(), $args = null ) {
			...
			if( 'form' == $this->form_tag ) {
				...
				if( ! empty( $this->form_action ) ) {
					$output .= ' action="' . esc_attr( $this->form_action ) . '"';
				}

				/* Form method */
				if( ! empty( $this->form_method ) ) {
					$output .= ' method="' . esc_attr( $this->form_method ) . '"';
				}
			}

			/* Additional form attributes */
			$output .= ' ' . self::render_attr( $this->form_attr );

			/* Opening form tag closing bracket */
			$output .=  '>';

			/* Optional before fields markup */
			$output .= $this->before_fields;

			/* Render tabs if there are fieldsets */
			if( $this->has_fieldsets() ) {

				$output .= '<div class="youxi-form-tabs">';

					$output .= $this->render_tabs_nav();

					$output .= '<div class="youxi-form-tabs-content">';

						/* Loop through each form object */
						foreach( $this->_form_objects as $object ) {

							if( $this->is_fieldset( $object ) ) {
								$output .= $this->render_fieldset( $object, $values );
							}
						}
						/* End loop */
				...
				foreach( $this->_form_objects as $object ) {
					...
					if( $this->is_field( $object ) ) {
						$output .= $this->render_field( $object, $values );
					}
				}
				/* End loop */
			}
			...
			$output .= $this->after_fields;

			/* Button Row */
			if( ! empty( $this->buttons ) ) {

				$output .= '<' . $this->button_row_tag . self::render_attr( $this->button_row_attr ) . '>';

				foreach( $this->buttons as $button ) {

					$button = wp_parse_args( $button, array( 'attr' => array(), 'text' => '' ));

					$output .= '<button' . self::render_attr( $button['attr'] ) . '>';
						$output .= esc_html( $button['text'] );
					$output .= '</button>';
				}

				$output .= '</' . $this->button_row_tag . '>';
			}
			...
			$output = '<div class="youxi-form-tabs-nav">';
				...
				foreach( $this->_form_objects as $object ) {
					...
					if( $this->is_fieldset( $object ) ) {
						...
						$output .= '<li' . ( ! empty( $object['active'] ) ? ' class="active"' : '' ) . '>';

							$output .= '<a href="#' . esc_attr( $object['id'] ) . '">';
		...
		public function render_field( $field, $values ) {
			...
			$output = $field->get_the_group_open( $this->group_tag, self::parse_attr( $this->group_attr, $field ) );
				...
				$output .= $field->get_the_control_open( $this->control_tag, self::parse_attr( $this->control_attr, $field ) );
					...
					/* Parse the value */
					$value = isset( $values[ $name ] ) ? $values[ $name ] : $field->get_option( 'std' );
		...
		public function render_fieldset( $fieldset, $values ) {
			...
			if( $this->is_fieldset( $fieldset ) ) {
				...
				extract( $fieldset, EXTR_PREFIX_ALL, 'fieldset' );

				$output .= '<fieldset id="' . esc_attr( $fieldset_id ) . '">';
		...
		public static function parse_attr( $attributes, $field ) {
			...
			foreach( $attributes as $attribute_name => $attribute_value ) {
				...
				$parsed_attributes[ $attribute_name ] = array();
				...
				if( is_string( $attribute_value ) ) {
					...
					foreach( $attribute_value as $index_or_val => $val_or_condition ) {
						...
						if( is_string( $val_or_condition ) ) {
							...
							$parsed_attributes[ $attribute_name ][] = $val_or_condition;
							...
							foreach( $val_or_condition as $option => $condition ) {
								...
								if( in_array( $field->get_option( $option ), $condition ) ) {
									...
									$parsed_attributes[ $attribute_name ][] = $index_or_val;
									break;
								}
							}
						}
		...
		public static function auto_fieldset() {
			return apply_filters( 'youxi_form_auto_fieldset', array(
				'id'    => 'general', 
				'title' => esc_html__( 'General', 'youxi' )
			));
		}
