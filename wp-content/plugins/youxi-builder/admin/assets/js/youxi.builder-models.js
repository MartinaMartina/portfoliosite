...
;(function( $, window, document, undefined ) {
	...
	builder.Helpers = {
		...
		getWrapperView: function( parent ) {
			...
			switch( parent.get( 'type' ) ) {
				...
				case 'row':
					var emptySlots = parent.get( 'slots' );
					if( emptySlots > 0 ) {
						return new builder.view.Column({
							parent: parent.view, 
							tag: builder.settings.columnShortcode, 
							size: emptySlots
						});
					}
			}
		...
		getShortcodeClass: function( shortcode ) {
			...
			if( shortcode == builder.settings.columnShortcode ) {
	...
	ColumnController = builder.controller.Column = BaseController.extend({
		...
		resize: function( e ) {
			...
			if( _.isElement( e.currentTarget ) ) {
				...
				if( 'increase' == act ) {
					...
					if( currentSize < builder.settings.rowSize ) {
						this.set( { size: currentSize + 1 }, { validate: true } );
					}
		...
		synchronize: function( model, sizeObject ) {
			...
			var sizeAtts = ColumnController.getDefaultSizeAtts(), 
				offsetAtts = ColumnController.getDefaultOffsetAtts();

			if( $.isPlainObject( sizeObject ) && _.has( sizeObject, sizeAtts ) ) {
				
				var props = { 'size': parseInt( sizeObject[ sizeAtts ] ) };

				if( _.has( sizeObject, offsetAtts ) ) {
					props['offset'] = parseInt( sizeObject[ offsetAtts ] );
				}
				...
				var atts = _.extend( {}, this.get( 'atts' ) );
				atts[ sizeAtts ] = parseInt( sizeObject );
		...
		getDefaultOffsetAtts: function() {
			return 'offset_' + builder.settings.columnDefaultType;
		}, 

		getDefaultSizeAtts: function() {
			return 'size_' + builder.settings.columnDefaultType;
		}, 

		getTitle: _.memoize( function( size ) {
			return l10n.columnTitlePrefix + [ size, builder.settings.rowSize ].join( '/' );
		})
	});
